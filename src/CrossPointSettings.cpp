...
CrossPointSettings::StatusBarSpec CrossPointSettings::statusBarSpec() const {
  ...
  // Upstream calls this field `clockFormat`; this port persists it as
  // `clockFormat12h` (same semantics: 0 = 24h, 1 = 12h). Keep our name — the
  // SettingsList persist key is "clockFormat12h", so renaming would orphan the
  // saved value.
  spec.clock12h = clockFormat12h == 1;
