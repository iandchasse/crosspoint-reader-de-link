...
Project: Open-source e-reader firmware for Xteink X4 (ESP32-C3), ported to a custom hardware platform for the ESP32-S3
Mission: Provide a lightweight, high-performance reading experience focused on EPUB rendering on constrained hardware.
S3 Mission: Keep code compatibility with C3-specific code for upstream merges and ongoing feature additions to base firmware,
but for new features can extend into PSRAM and use S3-specific capabilities that may help. 
...
* Anti-Hallucination: Do not assume the existence of libraries or ESP-IDF functions. If you are unsure of an API's availability for the ESP32-S3 target, check the open-x4-sdk or official docs first.
...
* RAM: ~380KB usable (VERY LIMITED - primary project constraint)
  * **NO PSRAM**: ESP32-C3 has no PSRAM capability (PSRAM will be preferred to not be used unless specified for an S3-specific feature)
...
1. **VS Code Extension** (Recommended):
   ...
   * This may be outdated info, as pioarduino is preferred to platformio, but the CLI commands will be the same
...
* **Environments** (in `platformio.ini`):
  * `s3_default`: Development (LOG_LEVEL=2, serial enabled)
  * `s3_gh_release`: Production (LOG_LEVEL=0)
  * `s3_gh_release_rc`: Release candidate (LOG_LEVEL=1)
  * `s3_slim`: Minimal build (no serial logging)
...
- **On S3**: ESP32's `SD_MMC` driver is natively thread-safe, so `HalFile` is aliased directly to `EspFsFile` without the mutex locking wrapper.
- **Never** call into `SdFat` / `SdSpiCard` / `FsBaseFile` / `SDCardManager` / raw `FsFile` directly — always use `HalStorage` and `HalFile` / `EspFsFile`.
