...
// the whole decoded image in one heap buffer. A full-page image (e.g. 480x800)
// needs ~96KB packed; streaming keeps the working set to a single MCU-row band
// so the cache always succeeds and the image is decoded exactly once, instead of
// re-decoding on every one of an image page's ~14 render passes (BW + AA restore
// + two grayscale planes x several strips) when the buffer can't be held.
//
// NOTE: on this S3 target (2MB PSRAM) the old full-image buffer also fit fine;
// this streaming form is kept to stay structurally aligned with upstream so the
// image pipeline stops conflicting on every merge. The band write path is
// orientation-independent (it stores image-local pixels); display orientation
// and tiled-grayscale strips are handled entirely on the render/replay side in
// DirectPixelWriter, which is left unchanged.
//
// Correctness relies on the decoder delivering rows in raster order (JPEGDEC:
// outer loop over y, inner over x; PNGdec: top to bottom). Consecutive blocks
// map to contiguous, non-overlapping destination row ranges, so once a block
// whose top row is Y arrives, every output row < Y is final and is flushed.
struct PixelCache {
