...
bool renderFromCache(GfxRenderer& renderer, const std::string& cachePath, int x, int y, int expectedWidth,
  ...
  // cachedHeight (~800) tiny reads through the storage layer + filesystem each
  // time — the dominant cost of displaying an image page. Batching rows into a
  // ~4KB buffer cuts that to ~20 reads per pass without holding the whole image.
