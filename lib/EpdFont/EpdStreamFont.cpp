...
static bool fsReadU8(EspFsFile& f, uint8_t& v) { return f.read(&v, 1) == 1; }
static bool fsReadU16(EspFsFile& f, uint16_t& v) { return f.read(reinterpret_cast<uint8_t*>(&v), 2) == 2; }
static bool fsReadU32(EspFsFile& f, uint32_t& v) { return f.read(reinterpret_cast<uint8_t*>(&v), 4) == 4; }
static bool fsReadI32(EspFsFile& f, int32_t& v) { return f.read(reinterpret_cast<uint8_t*>(&v), 4) == 4; }
static bool fsReadBytes(EspFsFile& f, void* buf, size_t n) {
...
EpdStreamFont* EpdStreamFont::load(const char* path, size_t cacheSizeBytes) {
  ...
  uint32_t magic = 0;
  uint8_t version = 0;
  if (!fsReadU32(file, magic) || magic != MAGIC) {
    LOG_ERR("STR", "Bad magic in %s", path);
    file.close();
    return nullptr;
  }
  if (!fsReadU8(file, version) || version != 1) {
  ...
  fsReadU8(file, advanceY);
  fsReadI32(file, ascender);
  fsReadI32(file, descender);
  fsReadU8(file, is2Bit);

  fsReadU32(file, intervalCount);
