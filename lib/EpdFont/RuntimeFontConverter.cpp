...
EpdFont* RuntimeFontConverter::generateEpdFontFromPath(const char* sdPath, int sizePt, bool is2Bit,
  ...
  // Allocate TTF to internal RAM temporarily
  uint8_t* ttfBuffer = (uint8_t*)heap_caps_malloc(size, MALLOC_CAP_8BIT);
  if (!ttfBuffer) {
    file.close();
    return nullptr;
  }
...
EpdFont* RuntimeFontConverter::generateEpdFont(const uint8_t* ttfBuffer, size_t ttfSize, int sizePt, bool is2Bit,
  ...
  // Allocate ONE giant block in internal RAM
  ...
  uint8_t* memBlock = (uint8_t*)heap_caps_malloc(total_alloc, MALLOC_CAP_8BIT);
  if (!memBlock) {
    LOG_ERR("RFC", "Memory alloc failed for %zu bytes", total_alloc);
    return nullptr;
  }
...
bool RuntimeFontConverter::generateAndSaveToFile(const char* sdTtfPath, int sizePt, bool is2Bit,
  ...
  uint8_t* ttfBuffer = (uint8_t*)heap_caps_malloc(ttfSize, MALLOC_CAP_8BIT);
  if (!ttfBuffer) {
    ttfFile.close();
    return false;
  }
