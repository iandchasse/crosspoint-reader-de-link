...
EpdFontFamily* EpdFontFileLoader::getFamily(SizeSlot slot) {
  ...
  // Allocate EpdFontFamily in internal RAM
  EpdFontFamily* fam = (EpdFontFamily*)heap_caps_malloc(sizeof(EpdFontFamily), MALLOC_CAP_8BIT);
  if (!fam) {
    LOG_ERR("FFL", "RAM alloc for EpdFontFamily failed");
    delete regular;
