...
EpdFont* EpdFontSerializer::loadFromFile(const char* path) {
  ...
  if (!readU16(file, klEntries) || !readU16(file, krEntries) || !readU8(file, klClasses) || !readU8(file, krClasses)) {
    free(tmpIntervals);
    free(tmpGlyphs);
    goto rd_err;
  }
  ...
  // --- single PSRAM alloc ---
  size_t total = szFont + szFontData + align4(intBytes) + align4(glyphBytes) + align4(kernLBytes) + align4(kernRBytes) +
                 align4(matrixBytes) + align4(ligBytes) + bitmapSize;
